...
// Store analyzed elements - SINGLE DECLARATION ONLY
var analyzedElements = {
  texts: [],
  images: [],
  videos: []
};
...
function extractTextContent() {
  ...
  const prioritySelectors = [
    ...
    'main p', 'div[role="main"] p', '.content p', '.article p', 'blockquote'
  ];
    ...
    try {
      ...
      elements.forEach(el => {
        ...
        if (text.length >= 100 && text.length <= 2000 && 
            ...
            !text.toLowerCase().includes('cookie') && !text.toLowerCase().includes('subscribe') && 
            !text.toLowerCase().includes('sign up') && !text.toLowerCase().includes('log in')) {
          textElements.push({ text: text, element: el });
        }
      });
    } catch (e) {
  ...
  if (textElements.length >= 3) {
    return removeDuplicates(textElements).slice(0, 10);
  }
  ...
  paragraphs.forEach(p => {
    ...
    if (text.length >= 100 && text.length <= 2000) {
      textElements.push({ text: text, element: p });
    }
  });
...
function extractVideos() {
  ...
  videoElements.forEach(video => {
    ...
    if (src) {
      videos.push({ url: src, element: video });
    }
  });

  const youtubeIframes = document.querySelectorAll('iframe[src*="youtube.com"], iframe[src*="youtu.be"]');
  youtubeIframes.forEach(iframe => {
    videos.push({ url: iframe.src, element: iframe, type: 'youtube' });
  });
    ...
    if (result.type === 'text') {
      analyzedElements.texts.forEach(item => {
        if (item.text === result.content) {
          ...
          tooltip.innerHTML = `<div class="verify-tooltip-header">⚠️ Potentially Fake Content</div><div class="verify-tooltip-content">Confidence: ${(result.confidence * 100).toFixed(1)}%</div>`;
      ...
      analyzedElements.images.forEach(item => {
        if (item.url === result.url) {
          ...
          overlay.innerHTML = `<div class="verify-overlay-badge">⚠️ Potentially Fake<br>${(result.confidence * 100).toFixed(0)}% confident</div>`;
      ...
      analyzedElements.videos.forEach(item => {
        if (item.url === result.url) {
          ...
          overlay.innerHTML = `<div class="verify-overlay-badge">⚠️ Potentially Fake Video<br>${(result.confidence * 100).toFixed(0)}% confident</div>`;
    ...
    chrome.runtime.sendMessage({
      ...
      if (response && response.results) {
        ...
        if (fakeItems.length > 0) {
          ...
          chrome.runtime.sendMessage({ action: 'updateBadge', count: fakeItems.length });
        } else {
      ...
      results.innerHTML = fakeItems.map((item, index) => `
        <div class="verify-comet-item">
          ...
          <div class="verify-comet-item-preview">
            ${item.type === 'text' ? truncateText(item.content, 120) : 'Potentially fake content detected'}
          </div>
        </div>
      `).join('');
  ...
  sidebar.innerHTML = `
    ...
    <div class="verify-comet-content">
      <div class="verify-comet-message">
        ...
        <div class="verify-comet-message-text">Found ${fakeItems.length} potentially fake item${fakeItems.length > 1 ? 's' : ''} on this page.</div>
      </div>
      <div class="verify-comet-results">
        ${fakeItems.map((item, index) => `
          <div class="verify-comet-item">
            ...
            <div class="verify-comet-item-preview">
              ${item.type === 'text' ? truncateText(item.content, 120) : 'Potentially fake content'}
            </div>
          </div>
        `).join('')}
      </div>
    </div>
    <div class="verify-comet-toggle">
  ...
  const significantChange = mutations.some(mutation => {
    return mutation.addedNodes.length > 0 && 
           Array.from(mutation.addedNodes).some(node => 
             node.nodeType === 1 && 
             (node.tagName === 'ARTICLE' || node.tagName === 'IMG' || node.tagName === 'VIDEO' ||
