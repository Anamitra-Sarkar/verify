...
const DUMMY_BADGES = [
    ...
    { name: 'Fact Champion', description: 'Top 100 contributors this month', rarity: 'Rare', color: 'text-blue-600', bgColor: 'bg-blue-50 dark:bg-blue-950', earned: false, earned_at: undefined },
    { name: 'Top Reporter', description: 'Submitted 250+ accurate reports', rarity: 'Rare', color: 'text-green-600', bgColor: 'bg-green-50 dark:bg-green-950', earned: true, earned_at: '2025-11-01T00:00:00Z' },
    { name: 'Vigilant Eye', description: 'First to report 50+ viral misinformation', rarity: 'Uncommon', color: 'text-orange-600', bgColor: 'bg-orange-50 dark:bg-orange-950', earned: false, earned_at: undefined },
    { name: 'Community Hero', description: 'Helped educate 1000+ users', rarity: 'Epic', color: 'text-pink-600', bgColor: 'bg-pink-50 dark:bg-pink-950', earned: false, earned_at: undefined },
];
...
// --- API functions now return DUMMY DATA. They are no longer async. ---
function fetchLeaderboard(): LeaderboardEntry[] {
  console.log('[DUMMY] Fetching leaderboard');
  return DUMMY_LEADERBOARD;
}
function fetchUserStats(): UserStats | null {
  console.log('[DUMMY] Fetching user stats');
  return DUMMY_USER_STATS;
}
function fetchBadges(): BadgeInfo[] {
  console.log('[DUMMY] Fetching badges');
  return DUMMY_BADGES;
}
function fetchDiscussions(): Discussion[] {
  console.log('[DUMMY] Fetching discussions');
...
export function CommunityPage({ language }: CommunityPageProps) {
  ...
  // This function now calls the dummy data functions. It does not need to be async.
  function loadData() {
    setLoading(true);
    try {
        setLeaderboardData(fetchLeaderboard());
        setUserStats(user ? fetchUserStats() : null);
        setBadgesData(user ? fetchBadges() : []);
        setDiscussions(fetchDiscussions());
    } catch (error) {
        console.error('Error loading dummy data:', error);
    } finally {
        // Use a small timeout to simulate loading, so the UI doesn't flash.
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              >
                <Card className="glass-card p-8">
                  ...
                  {leaderboardData.length >= 3 && (
                    <div className="flex items-end justify-center gap-6 mb-12">
                      <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex flex-col items-center">
                        <Avatar className="w-16 h-16 mb-3 ring-4 ring-gray-300"><AvatarFallback className="bg-gradient-to-br from-gray-400 to-gray-600 text-white">{leaderboardData[1].avatar}</AvatarFallback></Avatar>
                      ...
                      <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col items-center">
                        <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 2, repeat: Infinity }}>
                          <Avatar className="w-20 h-20 mb-3 ring-4 ring-yellow-400"><AvatarFallback className="bg-gradient-to-br from-yellow-400 to-yellow-600 text-white">{leaderboardData[0].avatar}</AvatarFallback></Avatar>
                        </motion.div>
                        <Badge className="mb-2 bg-gradient-to-r from-yellow-400 to-yellow-600 text-white"><Trophy className="w-3 h-3 mr-1" />1st</Badge>
                        ...
                        <div className="w-24 h-40 bg-gradient-to-br from-yellow-300 to-yellow-500 dark:from-yellow-600 dark:to-yellow-800 rounded-t-lg mt-4 relative overflow-hidden">
                          <motion.div className="absolute inset-0 bg-white" animate={{ opacity: [0, 0.3, 0] }} transition={{ duration: 2, repeat: Infinity }} />
                        </div>
                      </motion.div>
                      <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col items-center">
                        <Avatar className="w-16 h-16 mb-3 ring-4 ring-orange-300"><AvatarFallback className="bg-gradient-to-br from-orange-400 to-orange-600 text-white">{leaderboardData[2].avatar}</AvatarFallback></Avatar>
                  ...
                  {leaderboardData.length === 0 && (<div className="text-center py-8"><p className="text-gray-500 dark:text-gray-400">No leaderboard data available yet. Be the first to contribute!</p></div>)}
                  <div className="space-y-3">
                    {leaderboardData.map((user, idx) => (
                      <motion.div key={user.rank} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + idx * 0.1 }} className="glass-card rounded-xl p-4 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="w-8 text-center"><span className="font-display text-xl text-gray-500">#{user.rank}</span></div>
                            <Avatar className="w-12 h-12"><AvatarFallback>{user.avatar}</AvatarFallback></Avatar>
                            <div><p>{user.name}</p><p className="text-sm text-gray-500">{user.badge}</p></div>
                          </div>
                  ...
                  <>
                    <Card className="glass-card p-6">
                      ...
                      <div className="text-center mb-6">
                        <Avatar className="w-20 h-20 mx-auto mb-3"><AvatarFallback className="bg-gradient-to-br from-blue-500 to-purple-600 text-white text-2xl">{user.displayName?.substring(0, 2).toUpperCase() || user.email?.substring(0, 2).toUpperCase() || 'YU'}</AvatarFallback></Avatar>
                        <p className="mb-1">{user.displayName || user.email}</p>
                        <Badge variant="outline">Level {userStats.level}</Badge>
                      </div>
                      <div className="space-y-4">
                        <div>
                          <div className="flex justify-between mb-2"><span className="text-sm">Level {userStats.level}</span><span className="text-sm text-gray-500">{userStats.xp_current} / {userStats.xp_required} XP</span></div>
                          <Progress value={(userStats.xp_current / userStats.xp_required) * 100} className="h-2" />
                        </div>
                        <div className="grid grid-cols-2 gap-3 pt-4 border-t">
                          <div className="text-center"><p className="font-display text-2xl mb-1">{userStats.total_reports}</p><p className="text-xs text-gray-500">Reports</p></div>
                          <div className="text-center"><p className="font-display text-2xl mb-1">{userStats.accuracy}%</p><p className="text-xs text-gray-500">Accuracy</p></div>
                        </div>
                      </div>
                    </Card>
                    <Card className="glass-card p-6">
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Current Rank</span><span className="font-display">#{userStats.current_rank}</span></div>
                        <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">This Week</span><span className={userStats.rank_change >= 0 ? "text-green-600" : "text-red-600"}>{userStats.rank_change > 0 ? '+' : ''}{userStats.rank_change} {userStats.rank_change >= 0 ? '↑' : '↓'}</span></div>
                        <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Streak</span><span className="font-display">{userStats.streak_days} days 🔥</span></div>
                      </div>
                    </Card>
                  </>
                ) : (
                  <Card className="glass-card p-6"><div className="text-center py-8"><Users className="w-12 h-12 mx-auto mb-4 text-gray-400" /><p className="text-gray-600 dark:text-gray-400 mb-4">Login to see your stats and join the leaderboard!</p></div></Card>
                )}
              </motion.div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="badges" className="mt-8">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(badgesData).map((badge, idx) => {
                const iconMap: Record<string, any> = { 'Guardian Elite': Shield, 'Truth Seeker': Trophy, 'Fact Champion': Award, 'Top Reporter': Star, 'Vigilant Eye': TrendingUp, 'Community Hero': Users, };
                const Icon = iconMap[badge.name] || Shield;
                return (
                  <motion.div key={badge.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} whileHover={{ scale: 1.05, y: -5 }}>
                    <Card className={`${badge.bgColor} border-2 p-6 h-full relative ${badge.earned ? 'ring-2 ring-green-500' : 'opacity-75'}`}>
                      {badge.earned && (<div className="absolute top-2 right-2"><Star className="w-6 h-6 text-yellow-500 fill-yellow-500" /></div>)}
                      ...
                      {badge.earned && badge.earned_at && (<p className="text-xs text-green-600 mt-2">Earned {new Date(badge.earned_at).toLocaleDateString()}</p>)}
                    </Card>
                  </motion.div>
                );
              })}
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-4">
                {discussions.length > 0 ? discussions.map((discussion, idx) => (
                  <motion.div key={discussion.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
                  ...
                  <Card className="glass-card p-8">
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 mb-4">No discussions yet. Be the first to start one!</p>
                      {user && (<Button><Plus className="w-4 h-4 mr-2" />Start Discussion</Button>)}
                    </div>
                  </Card>
                )}
              </div>
              <div className="space-y-6">
                ...
                <Card className="glass-card p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['#Deepfakes', '#Politics', '#Health', '#Education', '#Technology', '#WhatsApp'].map((tag) => (<Badge key={tag} variant="outline" className="cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900">{tag}</Badge>))}
                  </div>
                </Card>
              </div>
            </div>
